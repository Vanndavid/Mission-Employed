...
export const JobApplications = ({
  ...
  return (
    <div className="space-y-8">
      ...
      <div role="tablist" aria-label="Job applications views" className="flex gap-1 border-b border-slate-200 dark:border-slate-700">
        {PAGE_TABS.map(({ key, label }) => {
          ...
          return (
            <button
              ...
              aria-controls={`jobs-panel-${key}`}
              onClick={() => selectTab(key)}
              className={`-mb-px px-4 py-3 text-sm font-bold border-b-2 transition-colors ${
                active
            ...
            >
              {label}
              {key === 'tracker' && (
                <span className="ml-2 text-xs font-bold text-slate-400">{applications.length}</span>
              )}
            </button>
          );
        })}
      ...
      {tab === 'tracker' && (
        <div role="tabpanel" id="jobs-panel-tracker" aria-labelledby="jobs-tab-tracker" className="space-y-6">
          ...
          {isAdding && (
            <div className="bg-white dark:bg-slate-800/80 rounded-2xl p-8 border border-slate-200 dark:border-slate-700 shadow-xl">
              <div className="space-y-4 max-w-2xl">
                <h3 className="text-lg font-bold text-slate-700 dark:text-slate-300">New application</h3>
                <input
                  autoFocus
                  aria-label="Company"
                ...
                <div className="flex justify-end gap-2">
                  <button
                    ...
                    className="px-6 py-3 rounded-xl font-bold text-slate-500 hover:text-slate-700 dark:hover:text-slate-300"
                  >
                    Cancel
                  </button>
                  <button
